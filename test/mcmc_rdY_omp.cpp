...
int main( int argc, char *argv[] )
{
  ...
  for ( int it_q2 = q2; it_q2 < q2 + 10; it_q2 ++ )
  {
    ...
    //string Y_name     = Y_filename       + string( "_" ) + to_string( it_q2 ) + string( ".txt" );
    ...
    /** Create a random row-permutation of Y. */
    vector<size_t> I( n ), J( 1 );
    for ( size_t i = 0; i < n; i ++ ) I[ i ] = i;
    for ( size_t j = 0; j < 1; j ++ ) J[ j ] = j;
    ...
    /** Execute MCMC. */
    mcmc::mcmc<T>( Y_perm, A, M, C1, C2, beta_m, alpha_a, pi_m, pi_a, 
